...
import json
import os
from sentinel_gate.interfaces.mcp_server import load_and_validate, infer_dataset_schema

async def test_mcp_logic():
    print("Testing MCP Tools Logic...")
    
    # Test infer_schema tool
    csv_path = os.path.join("sentinel_gate", "tests", "sample_data", "sample_data.csv")
    ...
    rules_path = os.path.join("sentinel_gate", "tests", "sample_data", "rules.json")
