...
"validations": {
  ...
  "volume_checks": { "min_rows": 100, "mandatory": true },
  "outlier_checks": { "columns": ["*"], "mandatory": false },
  "empty_string_checks": { "columns": ["nome"], "mandatory": true },
  "comparison_checks": [
    { "equation": "preco * qtd == total", "mandatory": true }
  ],
  "lookup_checks": [
    { "column": "id", "reference_source": {"file": "ref.csv"}, "reference_column": "id" }
  ],
  "unit_checks": [
    { "column": "peso", "unit": "kg", "max_magnitude": 1000 }
  ],
  "timestamp_checks": [
    { "column": "created_at", "format_type": "iso_timezone" }
  ]
}
```
...
| `date_check` | Valida se o formato da data é consistente com o esperado (ex: YYYY-MM-DD). | `{"column": "venda", "format": "%Y-%m-%d"}` |
| `duplicate_check`| Checa se existem linhas no dataset que são duplicatas exatas de outras. | `"duplicate_check": true` |
| `comparison_checks`| **Precisão**: Validação cruzada usando expressões (ex: A * B = C). | `{"equation": "preco * qtd == total"}` |
| `lookup_checks`| **Precisão**: Valida se valores existem em uma fonte externa de referência. | `{"column": "id", "reference_source": {...}}` |
| `unit_checks` | **Uniformidade**: Valida escalas e magnitudes de unidades de medida. | `{"column": "peso", "unit": "kg", "max_magnitude": 100}` |
| `timestamp_checks`| **Uniformidade**: Valida formatos Unix ou presença de Timezone (ISO). | `{"column": "ts", "format_type": "iso_timezone"}` |
...
python .\sentinel_gate\interfaces\cli.py --source csv --config .\seu_config_v6.json --output .\reports_test
```
