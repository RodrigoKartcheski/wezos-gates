...
class ReportGenerator:
    ...
    def generate_schema_html(cls, dataset_name: str, drift_report: Dict[str, Any], actual_schema: Dict[str, Any], output_file: str):
        ...
        html = f"""
        <html>
        <head><title>SentinelGate | Schema Report - {dataset_name}</title>{cls.BASE_STYLE}</head>
        <body>
            <div class='container'>
                <h1>SentinelGate Analysis: {dataset_name}</h1>
                <div class='card'>
    ...
    def generate_validations_html(cls, dataset_name: str, scores: Dict[str, float], results: List[Dict[str, Any]], aggregation_results: List[Dict[str, Any]], output_file: str):
        ...
        for r in results:
            ...
            # 3.18 STAFF FIX: Standardized output extraction (Metrics aware)
            # We flatten any nested dicts (like 'metrics') into the details column for compatibility
            detail_items = []
            for k, v in r.items():
                if k in ["type", "status", "column", "columns", "mandatory"]:
                    continue
                if isinstance(v, dict):
                    detail_items.extend([f"{sub_k}: {sub_v}" for sub_k, sub_v in v.items()])
                else:
        ...
        html = f"""
        <html>
        <head><title>SentinelGate | Validations - {dataset_name}</title>{cls.BASE_STYLE}
        <style>
        ...
        <body>
            <div class='container'>
                <h1>SentinelGate DQ Report: {dataset_name}</h1>
    ...
    def generate_discovery_html(cls, dataset_name: str, discovery_results: Dict[str, Any], output_file: str):
        ...
        html = f"""
        <html>
        <head><title>SentinelGate | Discovery - {dataset_name}</title>{cls.BASE_STYLE}</head>
        <body>
            <div class='container'>
                <h1>SentinelGate Insights: {dataset_name}</h1>
                {stats_html}
                {distinct_table}
...
<head>
    <meta charset="UTF-8">
    <title>SentinelGate Dashboard - {dataset_name}</title>
    <style>
...
<header>
    <h1>SentinelGate Dashboard: {dataset_name}</h1>
</header>
