...
class DiscoveryEngine:
    ...
    def detect_primary_keys(self, colunas_ignoradas: List[str] = None) -> Dict[str, Any]:
        ...
        if total_rows == 0:
            return {"status": "SKIPPED", "message": "Zero rows in dataset"}
        ...
        if candidates:
            res.update({"status": "SUCCESS", "type": "SINGLE", "columns": candidates, "distinct_count": total_rows})
            self.results["primary_key"] = res
            return res

        # 2. Iterative concatenation (Greedy approach for composite keys)
        colunas_combinadas = []
        max_search = min(len(colunas_ordenadas), 5)
        for r in range(2, max_search + 1):
            subset_cols = colunas_ordenadas[:r]
            distinct_combo = len(self.df[subset_cols].drop_duplicates())
            ...
            if distinct_combo == total_rows:
                res.update({"status": "SUCCESS", "type": "COMPOSITE", "columns": subset_cols, "distinct_count": distinct_combo})
                self.results["primary_key"] = res
                return res
    ...
    def check_deduplication(self, columns: List[str], label: str, excluded_columns: List[str] = None) -> Dict[str, Any]:
        ...
        if not valid_cols:
            return {"status": "SKIPPED", "message": "No valid columns for deduplication check"}
    ...
    def run_all(self, config: Dict[str, Any]):
        ...
        date_cols = config.get("date_analysis")
        if date_cols:
